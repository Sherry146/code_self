...
void insert_hash(Tree* pedigree) {//插入哈希表
	...
	strcpy_s(np->name, pedigree->name);
	np->next = alphabet[p];
	np->tree = pedigree;
	alphabet[p] = np;
}
...
void delete_hash(Tree* del) {//在哈希表中删除
	if (del == NULL)return;
	hashtable* de = alphabet[locate_hash(del->name)], * temp;
	for (; de->tree != del; de = de->next);
	if (de == NULL);
	else {
		if (de->next != NULL) {
			strcpy_s(de->name, de->next->name);
			de->tree = de->next->tree;
			temp = de->next;
			...
			if (alphabet[locate_hash(del->name)] == de) {
				free(alphabet[locate_hash(del->name)]);
				alphabet[locate_hash(del->name)] = NULL;
			}
			else {
				hashtable* np;
				for (np = alphabet[locate_hash(del->name)]; np->next != de; np = np->next);
				np->next = NULL;
...
Tree* common_ancestor_and_relationship(char left[], char right[], int* status) {//查找最近共同祖先和三代关系
	...
	if (lowone->level - lefthash->tree->level == 0 && lowone->level - righthash->tree->level == -1 ||
		lowone->level - lefthash->tree->level == -1 && lowone->level - righthash->tree->level == 0) {
		printf("直系，两人父母-子女关系\n"); return 	lowone;
	};
	if (lowone->level - lefthash->tree->level == 0 && lowone->level - righthash->tree->level == -2 ||
		lowone->level - lefthash->tree->level == -2 && lowone->level - righthash->tree->level == 0) {
		printf("直系，两人爷孙关系\n"); return 	lowone;
	};
	...
	if (lowone->level - lefthash->tree->level == -1 && lowone->level - righthash->tree->level == -2 ||
		lowone->level - lefthash->tree->level == -2 && lowone->level - righthash->tree->level == -1) {
		printf("旁系，两人伯叔-侄甥子女关系\n"); return 	lowone;
	};
